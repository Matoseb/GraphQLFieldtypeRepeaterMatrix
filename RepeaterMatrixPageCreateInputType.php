...
class RepeaterMatrixPageCreateInputType
{
  ...
  public static function getTemplateFields(Template $template)
  {
    ...
    // support for now
    $unsupportedFieldtypes = ['FieldtypeFile', 'FieldtypeImage'];
  ...
  public static function markRequiredTemplateFields(
    array $fields,
    Template $template
  ) {
    ...
    foreach ($template->fields as $field) {
      ...
      $fields[$field->name]['type'] = Type::nonNull(
    ...
    foreach ($values as $fieldName => $value) {
      ...
      if (!Permissions::canEditField($field, $page->template)) {
        throw new ValidationError(
          "You are not allowed to update field '{$field->name}'"
        );
      }
